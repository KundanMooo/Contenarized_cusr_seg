...
def load_data(filepath, sep='\t'):
    """Load the dataset from a file using the specified separator."""
...
def preprocess_data(df):
    ...
    df['Marital_Status'].replace({"Married": 1, "Together": 1, "Absurd": 0, "Widow": 0, "YOLO": 0, 
                                   "Divorced": 0, "Single": 0, "Alone": 0}, inplace=True)
    ...
    df = df[["Age", "Education", "Marital Status", "Parental Status", "Children", "Income", 
...
def process_data(input_filepath, output_path="./data/clustered_data.csv"):
    """
    Load the data from the input_filepath, preprocess it, perform PCA and clustering,
    then save the resulting DataFrame (with a 'cluster' column) to output_path.
    Returns the output_path.
    """
    # Load CSV using tab as the separator
    df = load_data(input_filepath, sep='\t')
    df = preprocess_data(df)
    
    preprocessor = preprocess_pipeline(df)
    scaled_data = preprocessor.fit_transform(df)
    # Recreate a DataFrame from the scaled data using the original column names
...
# For backward compatibility, you can still run datamain if needed
def datamain():
